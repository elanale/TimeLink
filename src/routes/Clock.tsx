...
interface TimeLog {
  id: string;
  clockIn?: { toDate: () => Date; seconds: number };
  clockOut?: { toDate: () => Date; seconds: number } | null;
  duration?: number;
}

// Route definition for /Clock
export const Route = createFileRoute("/Clock")({
  component: EmploymentClock,
});

export function EmploymentClock() {
  const { user } = useAuth();
  ...
  const handleClockOut = async () => {
    ...
    .map((docSnap) => ({
      ...(docSnap.data() as TimeLog),
      id: docSnap.id,
    }))
      .filter((log) => !log.clockOut)
      .sort((a, b) => (b.clockIn?.seconds ?? 0) - (a.clockIn?.seconds ?? 0))[0];

    if (incompleteLog) {
      const logDocRef = doc(db, "timeLogs", user!.uid, "logs", incompleteLog.id);
  ...
  const fetchLogs = async () => {
    ...
    snapshot.forEach((docSnap) => {
      ...
      fetchedLogs.push({
        id: docSnap.id,
        clockIn: clockIn ? { toDate: () => clockIn, seconds: Math.floor(clockIn.getTime() / 1000) } : undefined,
        clockOut: clockOut ? { toDate: () => clockOut, seconds: Math.floor(clockOut.getTime() / 1000) } : null,
        duration,
      });
    });
    ...
    setLogs(
      fetchedLogs.sort((a, b) => {
        ...
        return b.clockIn.seconds - a.clockIn.seconds;
      })
    );
  ...
  return (
    <div className="bg-white dark:bg-gray-900 p-6 rounded-xl shadow-md max-w-md mx-auto text-center space-y-6">
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Employment Clock</h1>
      ...
      <p>
        ...
        >
          {status === "idle" ? "Not clocked in" : status === "in" ? "Clocked In" : "Clocked Out"}
        </span>
      </p>
      ...
      <h3 className="text-lg font-semibold mt-6 text-white">Time Log History</h3>
            ...
            {logs.map((log) => (
              <tr key={log.id} className="border-t border-gray-600">
                <td className="px-4 py-2">
                  {log.clockIn ? log.clockIn.toDate().toLocaleString() : "--"}
                </td>
                <td className="px-4 py-2">
                  {log.clockOut ? log.clockOut.toDate().toLocaleString() : "--"}
                </td>
                <td className="px-4 py-2">
